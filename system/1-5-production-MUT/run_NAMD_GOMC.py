...
def write_namd_col_var(python_file_directory, path_namd_template, path_namd_runs, gomc_newdir,
                         run_no, box_number, namd_rst_dcd_xst_steps,
                         starting_pdb_box_x_file,
                         set_x_dim=None, set_y_dim=None, set_z_dim=None):
    """
    Writes the NAMD col var file in the NAMD run numbered folder
    ...
    Notes
    ---------
    Since the dummy atom to represent the origin is set at the beginning of the simulation, no volume moves are allowed!
    """
    ...
    if box_number == 0:
        ...
        namd_box_x_newdir = "{}/{}/{}{}_b".format(str(python_file_directory), path_namd_runs,
                                                  str(add_zeros_at_start_run_no_str), str(run_no))
    os.makedirs(namd_box_x_newdir, exist_ok=True)
    generate_namd_file = open("{}/colvars.col".format(namd_box_x_newdir), 'w')
    ...
    if run_no != 0:
        ...
        new_namd_data = namd_template_data.replace("pdb_box_file",
                                              "{}/Output_data_BOX_{}_restart.pdb"
                                              "".format(gomc_rel_path, str(box_number))
                                              )
    ...
    write_log_data = "NAMD colvar file for simulation number {} in box {} is " \
                     "completed \n".format(str(run_no), str(box_number))
    log_template_file.write(str(write_log_data))
    print(str(write_log_data))
